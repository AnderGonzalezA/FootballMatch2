...
public class TeamAndFootballMatch {
	public static void main(String[] args) {
		...
		if (args.length!=0) {
			...
			File teamsFile = new File("C:\\\\Users\\\\ik013043z1\\\\eclipse-workspace\\\\FootballMatch\\\\src\\\\Teams.txt");
						...
						if (it.next().getGoalsLocal()!=it.previous().getGoalsVisitor()) {
							...
							for (int j=0;j<teams.size();j++) {
								if (teams.get(j).getTeamName().equals(localTeam.getTeamName())) {
									System.out.print(teams.get(j).getTeamName() + "  " + teams.get(j).getCoach() + "  ");
									break;
								}
							}
